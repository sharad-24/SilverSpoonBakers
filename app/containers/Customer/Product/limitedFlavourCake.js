...
export default function LimitedFlavourCake(props) {
  ...
  const [selectedFlavours, setselectedFlavours] = useState({ flavours: null });
  ...
  const getPrice = () => {
    //console.log('getPrice', value.weight);
    const priceC =
      (Number(baseprice) + Number(form.flavour.price)) * Number(value.weight);
    setprice({ price: priceC });
  };

  const filterFlavour = () => {
    try {
          let res = props.allFlavours.filter(flavour => {
            if (props.product.flavours.includes(flavour._id)) {
              return flavour;
            }
          });
          //console.log({res});
          setselectedFlavours({ flavours: res });
          setForm({ ...form, flavour: res[0] });
        } catch (error) {
  ...
  const handleAddToCart = ({ weight }) => {
    try {
      ...
      console.log('payload: ', exisitingCartData);
