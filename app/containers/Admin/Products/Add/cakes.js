...
const Cakes = ({ productsAdd, allSubcategories, flavours, onAddProducts, onRemoveError}) => {
  ...
  const handleAddProduct = (values, onSubmitProps) => {
    try {
          ...
          if (Number(values.maxWeight) < Number(values.minWeight)) {
            throw new Error('Check min and max Weight');
          }
          ...
          console.log('length od s flavour:', selectedFlavours.length);
          if (hasAllFlavours) {
            const a = new Array();
            formData.append('flavours', null);
          } else if (!selectedFlavours.length > 0) {
            throw new Error('Please select a flavour');
          } else {
            formData.append(
              'flavours',
              JSON.stringify(selectedFlavours),
            );
          }

          formData.append('hasEgg', selectedIsEgg);
          formData.append('isCake', true);
          formData.append('isBestSeller', isBestSeller);
          formData.append('hasAllFlavour', hasAllFlavours);
                ...
                <Grid container spacing={1}>
                  <Grid item xs={12} sm={4} style={{ marginBottom: '20px' }}>
                    <TextField
                      ...
                      error={Boolean(errors.minWeight && touched.minWeight)}
                      helperText={
                        errors.minWeight &&
                        touched.minWeight &&
                  ...
                  <Grid item xs={12} sm={4} style={{ marginBottom: '20px' }}>
                    <TextField
                      ...
                      label="Max Weight"
                      variant="outlined"
                      error={Boolean(errors.maxWeight && touched.maxWeight)}
                      helperText={
                        errors.maxWeight &&
                        touched.maxWeight &&
                    ...
                    {flavours ?
                      ...
                      renderInput={params => (
                        <TextField
                          ...
                          label="Flavours"
                          placeholder="choose from list"
                          error={Boolean(errors.flavours && touched.flavours)}
                          helperText={
                            errors.flavours &&
                            touched.flavours &&
